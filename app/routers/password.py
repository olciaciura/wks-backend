...
conf = ConnectionConfig(
    MAIL_USERNAME = "olcia.ciura@gmail.com",      # Twój email
    MAIL_PASSWORD = "xhes bxpb uhah quvm",     # UWAGA: To NIE JEST hasło do konta! (czytaj niżej)
    MAIL_FROM = "olcia.ciura@gmail.com",          # Adres nadawcy widoczny dla użytkownika
    MAIL_PORT = 587,
...
async def forgot_password(req: ForgotPasswordRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    if user:
        ...
        #     <p>Cześć <strong>{user.login}</strong>,</p>
        #     <p>Otrzymaliśmy prośbę o zresetowanie hasła do Twojego konta. Kliknij w poniższy przycisk, aby ustawić nowe hasło. Link jest ważny przez 15 minut.</p>
